...
## Forked        2010 Anders Boesen Lindbo Larsen 
##                    <anders.bll AT gmail.com>
##                    Edited roughly to work standalone without Gajim.
##                    Also contains a few HTML layout fixes.
##
...
class HtmlHandler(xml.sax.handler.ContentHandler):
        ...
        def __init__(self, textview, startiter):
                ...
                self.starts_line_from_li_end = False
        ...
        def __length_tag_cb(self, value, tag, propname):
                try:
                        ...
                        pass#gajim.log.warn( "Error with prop: " + propname + " for tag: " + str(tag))
...
#                        gajim.log.error('Error loading image ' + str(ex))
        ...
        def _flush_text(self):
                ...
                if not self.preserve:
                        ...
                        text = whitespace_fx.sub(' ', text)
                        self.handle_specials(text)
                else:
...
#                        style += ';margin-left: 10px'
                ...
                if name == 'br':
                        ...
                        self.text = '&nbsp;'*len(self.list_counters)*3 + li_head + ' '
                        self._flush_text()
#                        self.starting = True
                elif name == 'dd':
                        self._jump_line()
        ...
        def endElement(self, name):
                ...
                if name == 'br':
                        ...
                        try:
                                ...
                                pass#gajim.log.debug(str('Error in hr'+e))
                elif name in LIST_ELEMS:
                        self.list_counters.pop()
                elif name == 'li':
                        if not self.starts_line_from_li_end:
                                newLine = True
                        self.starts_line_from_li_end = True
                elif name == 'img':
                        pass
                ...
                if newLine:
                        self._jump_line()
                self._end_span()
#                self.starts_line_from_li_end = name == 'li'
                #if not self._starts_line():
                #    self.text = ' '
